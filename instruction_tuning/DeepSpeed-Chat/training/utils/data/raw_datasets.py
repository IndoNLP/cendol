...
class PromptRawDataset(object):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name, use_auth_token=False):
        self.output_path = output_path
        self.seed = seed
        self.local_rank = local_rank
        if not dataset_name == 'local/jsonfile':
            self.raw_datasets = load_dataset(dataset_name, use_auth_token=use_auth_token)
...
class NusaT2TDataset(PromptRawDataset):
    def __init__(self, output_path, seed, local_rank, dataset_name, use_auth_token=False, sample_size=6005000, val_set_size=5000):
        ...
        # Sampling
        if sample_size > 0:
            self.raw_datasets["train"] = self.raw_datasets["train"].train_test_split(
                test_size=sample_size, shuffle=True, seed=42
            )['test']
    ...
    def get_rejected(self, sample):
        print(
            f"PromptDataset.get_rejected() - Warning: dataset {self.dataset_name} does not include rejected response."
        )
        return None

    def get_prompt_and_chosen(self, sample):
        return f'{sample["input"]} {sample["output"]}'

    def get_prompt_and_rejected(self, sample):
        print(
            f"PromptDataset.get_prompt_and_rejected() - Warning: dataset {self.dataset_name} does not include rejected response."
        )
        return None
