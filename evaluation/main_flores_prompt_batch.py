...
def to_prompt(input, prompt, prompt_lang, task_name, task_type, with_label=False, use_template=False):
    ...
    if task_type == Tasks.MACHINE_TRANSLATION.value:
        if 'flores' in task_name:
            ...
            src_lang = task_names[1].replace('_Latn', '')
            tgt_lang = task_names[2].replace('_Latn', '')            
        else: # Nusantara Schema
...
if __name__ == '__main__':
    if len(sys.argv) != 5:
        raise ValueError('main_flores_prompt.py <prompt_lang> <model_path_or_name> <n_shot> <n_batch>')
    ...
    nlg_datasets = load_flores_datasets()
    ...
    for i, dset_subset in enumerate(nlg_datasets.keys()):
        ...
        elif 'dev' in nlg_dset.keys():
            few_shot_data = nlg_dset['dev']
    ...
    pd.DataFrame.from_dict(metrics).reset_index().to_csv(f'{metric_dir}/flores_results_{prompt_lang}_{N_SHOT}_{MODEL.split("/")[-1]}.csv', index=False)
