...
def run_app():
    ...
    if mode == "Helicopter view":
        ...
        # Initialize NusaCrowd config helper
        ...
        print(dataset_list)
        tydiqa_id_index = dataset_list.index("tydiqa_id")

        #
        # Select a dataset - starts with tydiqa_id
        #
        dataset_key = st.sidebar.selectbox(
            "Dataset",
            dataset_list,
            key="dataset_select",
            index=tydiqa_id_index,
        ...
        if dataset_key is not None:
            ...
            #
            def get_dataset_confs(conhelps, dataset_key):
                dset = conhelps.filtered(lambda x: dataset_key in x.dataset_name)
                return [dset_i.config for dset_i in dset]

            configs = get_dataset_confs(conhelps, dataset_key)
            conf_option = None
            if len(configs) > 0:
